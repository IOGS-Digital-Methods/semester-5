...
# CAS RGB
image_rgb = cv2.imread('./_data/bricks2.jpg')
image_rgb = cv2.cvtColor(image_rgb, cv2.COLOR_BGR2RGB)


blurred_image_1 = cv2.GaussianBlur(image_rgb, kernel_size, 2)
blurred_image_2 = cv2.blur(image_rgb, kernel_size)

# Display images
fig, ax = plt.subplots(nrows=2, ncols=3)
ax[0,0].imshow(image_rgb)
ax[0,0].set_title('Initial RGB Image / Filter')
ax[0,1].imshow(blurred_image_1, cmap='gray')
ax[0,1].set_title('Gaussian Blur')
ax[0,2].imshow(blurred_image_2, cmap='gray')
ax[0,2].set_title('Box blur')

ax[1,1].imshow(image_rgb - blurred_image_1, cmap='gray')
ax[1,1].set_title('Gaussian Blur')
ax[1,2].imshow(image_rgb - blurred_image_2, cmap='gray')
ax[1,2].set_title('Box blur')
