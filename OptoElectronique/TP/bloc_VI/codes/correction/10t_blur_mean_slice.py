...
# Image
image = cv2.imread('./step_by_step/_data/noise_vi_small.png', cv2.IMREAD_GRAYSCALE)
...
##
# FFT
fft0 = np.fft.fftshift(np.fft.fft2(image))
fft1 = np.fft.fftshift(np.fft.fft2(blurred_image_1))
fft2 = np.fft.fftshift(np.fft.fft2(blurred_image_2))
eps = 0.01

# Display Slice Gradient
fig, ax = plt.subplots(nrows=2, ncols=3)
ax[0,0].imshow(np.log(np.abs(fft0) + eps), cmap='gray')
ax[0,0].set_title('Initial Gray Image / Filter')
ax[0,1].imshow(np.log(np.abs(fft1) + eps), cmap='gray')
ax[0,1].set_title(f'Gaussian Blur')
ax[0,2].imshow(np.log(np.abs(fft2) + eps), cmap='gray')
ax[0,2].set_title(f'Box blur')

ax[1,0].imshow(image, cmap='gray')
ax[1,0].set_title('Initial Gray Image / Filter')
ax[1,0].axhline(y=line_number, color='red', linewidth=2)
ax[1,1].imshow(blurred_image_1, cmap='gray')
ax[1,1].set_title(f'Gaussian Blur')
ax[1,2].imshow(blurred_image_2, cmap='gray')
ax[1,2].set_title(f'Box blur')
plt.show()
