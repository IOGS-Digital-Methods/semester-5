...
{ \Large \bfseries\color{violet_iogs} TP2 - Bases de traitement d'images sous OpenCV \\[0.4cm] }
...
\begin{center}
	\includegraphics[width=0.6\textwidth]{../images/images_array_gray_rgb.png}
\end{center}
...
\Manip Remplir le \textbf{tableau VI-5} de la feuille de résultats pour une image en nuance de gris et une image RGB, en précisant : 
\begin{itemize}
	\item la taille de la matrice dans le cas des deux images
	\item le nombre de canaux
	\item le type de données stockées dans ces matrices
\end{itemize}

\Quest Dans le cas d'une image en RGB, à quelles couleurs correspondent chacun des canaux ?
...
\Manip Remplir le \textbf{tableau VI-6} de la feuille de résultats pour une image en nuance de gris et une image RGB, en précisant : 
\begin{itemize}
	\item si l'opération est possible
	\item l'effet visualisé sur l'image résultante
\end{itemize}
...
\subsection{Moyenne des images}

\Manip Pour les \textbf{valeurs de contraste précisées dans la feuille de résultats} ($\alpha = 0.5$ et $\alpha = 1.2$), pour une image en nuance de gris :

\begin{itemize}
	\item Calculer la moyenne de tous les pixels de l'image originale
	\item Calculer la moyenne de tous les pixels de l'image modifiée
\end{itemize} 

\subsection{Affichage des images avec \textit{Matplotlib.pyplot}}

\Quest L'affichage de la nouvelle image est-il correct ?

\Manip Réafficher l'image avec les options suivantes de \fbox{\texttt{imshow}} : \fbox{\texttt{vmin=0}} et \fbox{\texttt{vmax=255}}

\Quest Que pouvez-vous conclure sur la méthode \textit{imshow} de la bibliothèque \textit{Matplotlib.pyplot} ? 
...
\Manip Ouvrir une image RGB et appliquer les mêmes effets. Que pouvez-vous conclure ?

\Manip Remplir le \textbf{tableau VI-7} de la feuille de résultats en précisant : 
\begin{itemize}
	\item si l'opération est possible
	\item l'effet visualisé sur l'image résultante
\end{itemize}

\Quest A partir de ces observations, quelle est la méthode la plus fiable pour binariser une image afin de détecter des éléments (objets) dans une scène ?
...
\Manip Générer une ellipse de taille 31 par 21 pixels et afficher ce noyau.
...
\Quest De quel type est l'image utilisée pour ces opérations ?
...
\Manip Ouvrir une image RGB (\textit{formes\_bruit.png par exemple}) et appliquer les mêmes effets. Que pouvez-vous conclure ?

\Manip Ouvrir cette même image en niveau de gris et appliquer les mêmes effets. Que pouvez-vous conclure ?

\Manip Synthétiser vos résultats dans le \textbf{tableau VI-8} de la feuille de résultats. 
...
\Manip Remplir le \textbf{tableau VI-9} de la feuille de résultats, pour une image en nuance de gris et en RGB, en précisant : 
\begin{itemize}
	\item si l'opération est possible
	\item la taille des données de la FFT
	\item le type de données contenues dans la matrice (FFT)
\end{itemize} 

\subsection{Transformation de l'image via la FFT}
...
\Quest Commenter les effets des deux méthodes utilisées pour lisser l'image. Quelle méthode est utilisée ici pour montrer de manière objective les modifications apportées à l'image ? 

\Quest Que se passe-t-il sur une image RGB ? 

\Manip Synthétiser vos résultats dans le \textbf{tableau VI-10} de la feuille de résultats. 
...
%%%%%%%%%%%%%    Etape H
\section{H - Appliquer un filtre passe-haut sur une image {[40 min]}}
...
\Manip Synthétiser vos résultats dans le \textbf{tableau VI-11} de la feuille de résultats. 
