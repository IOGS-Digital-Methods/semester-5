...
\Quest Quelles sont les valeurs minimale et maximale que peuvent prendre les pixels de la caméra ? Quelle est alors la \textbf{profondeur binaire} (\textit{i.e.} le nombre de bits sur lequel est codé chacun des pixels) de la caméra utilisée ?
...
On souhaite à présent remplir le \textbf{tableau VI-1} de la feuille de résultats et mesurer l'écart d'intensité relevé dans la zone centrale du champ de vision pour 3 hauteurs de travail différentes.

\Manip Sélectionner une zone d'intérêt de 800 x 800 pixels au centre du champ de vision de la caméra.

\Manip Mesurer les niveaux de gris minimal et maximal obtenus pour \textbf{3 hauteurs de travail différentes} : 25cm, 14cm et 6cm (environ) entre le fond et la caméra. 
...
\Quest Comparer vos observations avec la documentation partielle du constructeur (fournie dans le document \textit{Ressources}).
...
On souhaite vérifier la linéarité du capteur en fonction du temps d'intégration, pour les 3 couleurs d'éclairage ainsi que du blanc. On souhaite alors remplir le \textbf{tableau VI-2} de la feuille de résultats.
...
\section{F - Résolution et contraste {[30 min]}}

\Quest A partir des dimensions mesurées précédemment (champ de vision) et de la taille du capteur, quelle est la taille minimale qu'il est possible de mesurer à l'aide de ce système de vision (côté objet) ?
...
Pour vérifier la résolution spatiale du système de vision, nous allons \textbf{utiliser des mires} (type Foucault). Ces mires sont des répétitions spatiales d'un même motif (succession de bandes noires et blanches) espacé d'une distance connue.
...
On souhaite remplir le \textbf{tableau VI-3} de la feuille de résultats.

\Manip Mesurer la hauteur des variations (valeur minimale et valeur maximale) pour différents espacements des bandes.

\Manip Calculer l'écart entre la valeur maximale et la valeur minimale puis la valeur normalisée. \textit{On normalisera les données par rapport à l'écart pour un pas de 2~mm.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{G - Première modélisation {[60 min]}}
...
On souhaite remplir les \textbf{tableaux VI-4a et VI-4b} de la feuille de résultats, pour deux temps d'intégration différents (sans saturation).
...
\Manip Refaire cette expérience pour un second temps d'intégration différent du premier (temps divisé par deux par exemple).

\Quest Ces mesures sont-elles dépendantes du temps d'intégration ?

\Quest Cette méthode de discrimination des cubes en fonction de leur couleur est-elle robuste à un changement de temps d'intégration ? Que proposez-vous comme amélioration ?
