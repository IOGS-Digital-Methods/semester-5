...
5N-027-SCI \qquad \textbf{\Large TP Séance 3}
...
Lors de cette troisième séance, vous allez :
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item réaliser un \textbf{asservissement} de l'intensité lumineuse
	\end{itemize} 
\end{itemize} 

\textit{Vous serez amenés à utiliser des maquettes déjà précâblées pour l'ensemble de ces séances. Une description des maquettes est fournie par la suite.}
...
Pour cela, on souhaite pouvoir remplir le tableau suivant (\textbf{à reproduire dans votre cahier de laboratoire}) pour 3 valeurs de résistance $R_{PHD}$ différentes ($10\operatorname{k\Omega}$, $100\operatorname{k\Omega}$ et $1\operatorname{M\Omega}$) et \textbf{caractériser la réponse en fréquence et en temps des trois circuits} suivants (première maquette) : photodétection simple, photodétection avec suiveur, montage transimpédance.
...
\begin{tabular}{|c|c|c|c|c|c|}
  ...
  transimpédance & & & & \\
  \hline
  transimpédance avec filtrage & & & & \\
  \hline
\end{tabular}
\end{center}

où $|V_s|_{MAX}$ est l'amplitude maximale du signal de sortie, [BP] est la bande-passante à $-3\operatorname{dB}$ du système, [BP]*$R_{PHD}$ le produit de la bande-passante par la valeur de la résistance $R_{PHD}$ et le temps de réponse à 95\% du système.

\bigskip

Vous devrez également vous intéresser à l'\textbf{impact de la tension de polarisation} sur les performances dynamiques du montage (bande-passante et amplitude maximale).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Montage 1 / Circuit simple de photodétection}
...
\textit{Voir le schéma du montage dans le sujet du TP2.}

\newpage
\subsection{Montage 2 / Suiveur}
...
\begin{circuitikz}
	...
	% fleche
	\draw (-0.5,0.3) edge[->, green!40!black] (-0.5,4.2);
	\node[text=green!40!black] (Ein) at (-1,2.25){$E_P$};
...
\subsection{Montage 3 / Transimpédance}
...
\begin{circuitikz} 
	...
	\draw[->, thick, blue!40!black, decorate, decoration={snake, amplitude=1mm, segment length=4mm}] (-3.5,2.25) -- (-3.5,1.25);
	\node[text=blue!40!black] (Phi) at (-4,1.75){$\Phi_e$};
...
\subsection{Montage 4 / Transimpédance avec filtrage}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Maquette 1 / Étude comparative de 4 montages de photodétection}

Il s'agit dans cette séance de \textbf{caractériser des circuits de photodétection} et \textbf{comparer leurs performances dynamiques} (fréquence et temps).

\subsection{Travail à réaliser}

\bigskip

Il est demandé pour chacun des montages (et pour les 3 valeurs de résistance $R_{PHD}$ proposées : $10\operatorname{k\Omega}$, $100\operatorname{k\Omega}$ et $1\operatorname{M\Omega}$) : 

\begin{itemize}%[label=$\square$]
	\item de \textbf{définir les liens théoriques} entre la tension de sortie $V_S$ et le flux lumineux capté par la photodiode $\Phi_e$ (par l'intermédiaire du courant imposé par la photodiode - $I_{photo}$)
	\item de \textbf{rédiger les protocoles utilisés} (avec schéma de câblage, paramètres des instruments de mesure et étapes expérimentales) pour caractériser les montages
	\item de \textbf{relever le diagramme de Bode} (mode automatique) ainsi que la réponse à un échelon
	\item de \textbf{mesurer les grandeurs caractéristiques} suivantes : bande-passante, temps de réponse, fréquence de résonance...
	\item d'analyser ces différents résultats
\end{itemize}

\bigskip

Vous devrez \textbf{proposer un livrable final} qui présente une \textbf{étude comparative des différents montages} en insistant sur la dépendance des grandeurs caractéristiques avec la valeur de la résistance $R_{PHD}$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Maquette 1 / Description}

\textit{Le schéma de la maquette est fournie en annexe de ce document.}


\noindent \rule{\linewidth}{1pt}

La maquette doit-être alimentée par une alimentation symétrique de +/- 15V.

\noindent \rule{\linewidth}{1pt}

\begin{center}
\begin{tabular}{|>{\centering\arraybackslash}p{2.5cm}|>{\centering\arraybackslash}p{2.5cm}|>{\centering\arraybackslash}p{10cm}|}
  
  \Xhline{2pt}
  Signal & Direction & Description\\
  \Xhline{2pt}
  \textbf{IN1} & Entrée & Signal appliqué sur les LED \\
  \hline
  \textbf{IN2} & Entrée &  \\
  \Xhline{2pt}
  \textbf{OUT1} & Sortie & Sortie du montage transimpédance \\
  \hline
  \textbf{OUT2} & Sortie & Sortie du montage simple avec suiveur \\
  \hline
  \textbf{OUT3} & Sortie & Sortie du montage de photodétection simple \\
  \hline
  \textbf{OUT4} & Sortie & Image du courant dans les LED ($R_{LED} = 270 \Omega$) \\
  \Xhline{2pt}
  \textbf{POT 15} & Potentiomètre & Réglage de la tension de polarisation ($E_P$) \\
  \Xhline{2pt}
\end{tabular}
\end{center}
...
\textbf{3 valeurs de résistance !}

Il existe 3 versions de cette maquette, pour chacune des valeurs de résistance à comparer. Vous devrez donc reprendre le même travail sur ces 3 versions afin d'en extraire des données et ainsi analyser l'impact de la résistance $R_{PHD}$ sur chacun des montages.
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Maquette 2 / Asservissement d'intensité lumineuse}

Un système de photodétection permet de mesurer l'intensité lumineuse résultant d'un ensemble de sources à un endroit donné. Nous allons à présent utiliser l'un des montages étudiés précédemment comme capteur de luminosité dans un système d'asservissement de l'intensité lumineuse d'une LED.

\textit{Les différents étages de ce système ont été (ou seront - selon les groupes) étudiés en TD, ainsi que l'impact d'une boucle d'asservissement sur un système.}
...
\subsection{Maquette 2 / Description}
...
\begin{tabular}{|>{\centering\arraybackslash}p{2.5cm}|>{\centering\arraybackslash}p{2.5cm}|>{\centering\arraybackslash}p{10cm}|}
  ...
  Signal & Direction & Description\\
  \Xhline{2pt}
  \textbf{IN1} & Entrée & Consigne d'intensité lumineuse $E_C$ \\
  \hline
  \textbf{IN2} & Entrée & Perturbation (LED secondaire) \\
  \Xhline{2pt}
  \textbf{OUT1} & Sortie & Mesure en sortie du système de photodétection $m$ \\
  \hline
  \textbf{OUT2} & Sortie & Image du courant dans la LED D1 ($R_{LED} = 270\operatorname{\Omega}$) \\
  \hline
  \textbf{OUT3} & Sortie & Erreur entre consigne et mesure $\varepsilon$ \\
  \hline
  \textbf{OUT4} & Sortie & Sortie du correcteur PID $u$ \\
  \Xhline{2pt}
  \textbf{POT 5} & Potentiomètre & Réglage de la tension d'offset sur la LED D1 \\
  \Xhline{2pt}
\end{tabular}
\end{center}
...
\textit{Le sujet est disponible sur le site du LEnsE.}