...
for ka in K_A:
    # Correcteur P seul
    sys_A = create_gain(ka)
    sys_P = create_pid(kp=1)
    sys_M = create_gain(1)
    sys_fb_P = create_closed_loop(sys_A, sys_P, sys_M)
    results_ol[ka] = ct.step_response(sys_fb_P, time_pts)
...
for kp, response in results_ol.items():
    plt.plot(time_pts * 1e6, response.outputs, label=f'KA = {kp}')
plt.xlabel('Time (µs)')
plt.ylabel('Amplitude')
plt.title('Step response - Different gain for A')
plt.ylim(-0.1, 1.5)
plt.legend()
plt.grid(True)
